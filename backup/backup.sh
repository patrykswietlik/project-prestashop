...
DB_CONTAINER="prestashop-default-db"
PRESTASHOP_CONTAINER="prestashop-default"
...
echo "Copying 'themes', 'img' and 'modules' folders"
docker cp "$PRESTASHOP_CONTAINER:/var/www/html/$THEMES_FOLDER" "$DESTINATION_FOLDER"
if [ $? -ne 0 ]; then
    echo "Error when copying a folder $THEMES_FOLDER!" >&2
    exit 1
fi
